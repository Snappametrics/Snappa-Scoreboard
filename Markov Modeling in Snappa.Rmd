...
off_transition_list = offensive_scores %>%
  ...
  map(function(game){
    ...
    running_score = table$running_score %>% append(0, after = 0)
...
off_transition_list = game_team_pair %>% 
  pull(game_id) %>%
  map(function(game){
...
def_transition_list = game_team_pair %>%
  pull(game_id) %>%
  map(function(game){
    rounds_in_game = total_rounds %>%
      filter(game_id == game) %>%
  ...
  # In order to model the transition from the initial state, a score = 0,
  # I add the first 0 to the vector
    running_score = table$running_score %>% append(0, after = 0)
    # Create a matrix that will be used for our transition counts
    for (i in 2:length(running_score)){
      old_score = running_score[i - 1]
...
# from one state to another. Not ideal, but oh well
transition_backup = function(player_stats, scores, player_id_1, player_id_2, 
                              player_id_3 = NA, player_id_4 = NA){
...
# Prelude to the games. Get the transition_probabilities and the backups
team_A_transitions = transition_probabilities(player_stats, scores, 9, 22)
team_B_transitions = transition_probabilities(player_stats, scores, 2, 4)
team_A_backup = transition_backup(player_stats, scores, 9, 22)
team_B_backup = transition_backup(player_stats, scores, 2, 4)



markov_single_game = function(A_team_size, B_team_size){
 # We'll track scores between teams according to a time series of each team's score.
  # The first score for both teams is 0
      team_A_history = c(0) 
      team_B_history = c(0)
...
# obtain the transition probs
team_A_transitions = transition_probabilities(player_stats, scores, team_A[1], team_A[2], team_A[3], team_A[4])
team_A_backup = transition_backup(player_stats, scores, team_A[1], team_A[2], team_A[3], team_A[4])
team_B_transitions = transition_probabilities(player_stats, scores, team_B[1], team_B[2], team_B[3], team_B[4])
team_B_backup = transition_backup(player_stats, scores, team_B[1], team_B[2], team_B[3], team_B[4])
...
# a sequence to iterate
  ...
  games_record = c(seq(1, iterations)) %>% map(function(iteration_number){
    markov_single_game(A_size, B_size)
})
return(games_record)
}

# Test command
games_list = markov_simulate_games(c(1,2), c(1,2) , 1000)


# See the win percentage of the teams
who_won = games_list %>% 
  map(function(list){
    list$won
  }) 

team_A_winrate = length(who_won[who_won == "A"]) / length(games_list)
team_B_winrate = length(who_won[who_won == "B"]) / length(games_list)



# For sanity, test the stability of the estimations. I think that doing several
# thousand simulations many times over and taking the average of those win rates is
# the move



seq(1,50) %>% map(function(number){
  
  games_list = markov_simulate_games(c(2,9), c(2,9), 500)
})
```